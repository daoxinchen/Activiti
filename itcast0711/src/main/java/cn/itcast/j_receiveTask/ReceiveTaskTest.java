...
public class ReceiveTaskTest {
	...
	public void startProcessInstance(){
		...
		Execution execution1 = processEngine.getRuntimeService()
					...
					.activityId("receivetask1")//当前活动的id，对应receiveTask.bpmn文件中的活动节点的id的属性值
					.singleResult();
		...
		Execution execution2 = processEngine.getRuntimeService()
					.createExecutionQuery()//创建执行对象查询
					.processInstanceId(pi.getId())//使用流程实例ID查询
					.activityId("receivetask2")//当前活动的id，对应receiveTask.bpmn文件中的活动节点的id的属性值
					.singleResult();
		...
		System.out.println("给老板发短信：金额时："+value);
